...
import { composeMailSchema, inboundMailSchema } from "../../apps/web/src/lib/schemas/mail-message.schemas";
...
describe("Multivrs Mail boundaries", () => {
  ...
  test("requires HTTPS webhook endpoints", () => {
    expect(createMailWebhookSchema.safeParse({ url: "http://example.com/hook", events: ["email.sent"] }).success).toBe(false);
    expect(createMailWebhookSchema.safeParse({ url: "https://example.com/hook", events: ["email.sent"] }).success).toBe(true);
  });

  test("validates normalized provider and inbound events", () => {
    expect(mailProviderEventSchema.parse({ providerEventId: "evt_1", providerMessageId: "msg_1", type: "delivered" }).type).toBe("delivered");
    expect(inboundMailSchema.safeParse({ providerEventId: "evt_1", mailbox: "support@example.com", messageId: "<one@example.com>", from: "person@example.com", to: ["support@example.com"] }).success).toBe(true);
  });
