...
import { Bell, Check, Globe, Server, ShieldAlert, Users } from "lucide-react";
...
export function NotificationDropdown() {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="end"
        className="w-[380px] p-0 rounded-[24px] border-white/10 bg-background/95 backdrop-blur-xl shadow-2xl overflow-hidden card-grain"
      >
          ...
          <div className="p-2 space-y-1">
            {MOCK_NOTIFICATIONS.map((notification) => {
              ...
              return (
                <DropdownMenuItem
                  key={notification.id}
                  className="p-3 rounded-[16px] flex items-start gap-3 cursor-pointer group hover:bg-white/4 transition-colors focus:bg-white/4"
                >
                  <div
                    className={cn(
        ...
        <div className="p-2 border-t border-white/5 bg-white/1">
          <Link href="/dashboard/notifications">
            <Button
              variant="outline"
              className="w-full py-2.5 rounded-[14px] text-[12px]"
            >
