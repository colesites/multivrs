...
export function NotificationsPage() {
  ...
  return (
    <div className="space-y-8 relative z-10 animate-in fade-in slide-in-from-bottom-[5%] duration-700 ease-out fill-mode-both max-w-[1200px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mt-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            variant="outline"
            size="sm"
            className="flex items-center gap-2 px-4 py-2 text-[11px] text-rose-400/80 hover:text-rose-400"
          >
      ...
      {/* Main Container */}
      <div
        className="rounded-[28px] border border-border bg-background relative overflow-hidden card-grain inner-glow shadow-2xl"
        style={
          { "--glow-color": "rgba(168, 85, 247, 0.08)" } as React.CSSProperties
        }
      >
        {/* Ambient glow */}
        ...
        <div className="flex items-center border-b border-white/4 p-2 bg-white/1">
          <Button
            variant="ghost"
            size="sm"
            className="px-5 py-2.5 text-xs bg-white/4 rounded-xl shadow-sm text-foreground"
          >
          ...
          <Button
            variant="ghost"
            size="sm"
            className="px-5 py-2.5 text-xs text-muted-foreground hover:text-foreground transition-colors flex items-center gap-2"
          >
            Unread
            {unreadCount > 0 && (
          ...
          <Button
            variant="ghost"
            size="sm"
            className="px-5 py-2.5 text-xs text-muted-foreground hover:text-foreground transition-colors"
          >
            Archived
          </Button>
        </div>
        ...
        <div className="divide-y divide-white/3 relative z-10">
          {MOCK_NOTIFICATIONS.map((notification) => {
            ...
            return (
              <div
                key={notification.id}
                className="group flex flex-col sm:flex-row sm:items-center gap-4 p-5 hover:bg-white/2 transition-colors duration-300"
              >
                {/* Icon */}
                <div
                  className={cn(
                    ...
                    color,
                    !notification.read && "shadow-[0_0_15px_currentColor]",
                  )}
                >
                ...
                <div className="flex sm:flex-col items-center sm:items-end justify-between gap-3 shrink-0 ml-12 sm:ml-0">
                  ...
                  <div className="flex items-center gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                    ...
                    <Button
                      size="xs"
                      className="h-8 px-3 bg-foreground text-background hover:bg-foreground/90 text-[10px] flex gap-1.5"
                    >
