  ...
  return (
    <div className="rounded-2xl bg-white/1.5 border border-white/5 p-4 flex items-center gap-4 hover:bg-white/3 hover:border-white/8 transition-all duration-300 group">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-[0.15em] mb-0.5">
          {label}
        </p>
        <div className="flex items-baseline gap-2">
          <span className="text-lg font-black tabular-nums text-foreground tracking-tight">
            {value}
          </span>
      ...
      <div
        className={cn("h-2 w-2 rounded-full shrink-0", ACCENT_MAP[color].dot)}
      />
    </div>
  );
}
