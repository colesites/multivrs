...
export function MailOverview({
  ...
  return (
    <div className="w-full space-y-8 px-5 py-8 lg:px-8">
      <section className="relative overflow-hidden rounded-2xl border border-white/10 bg-[radial-gradient(circle_at_10%_0%,rgba(37,99,235,.15),transparent_34%),linear-gradient(130deg,#0c1015,#08090c)] p-6 md:p-8">
      ...
      <section className="grid gap-px overflow-hidden rounded-xl border border-white/[0.08] bg-white/[0.08] sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map(([label, value, Icon]) => (
          <div className="bg-[#090b0f] p-5" key={label}>
      ...
      <section className="grid gap-4 md:grid-cols-[1.35fr_.65fr]">
        <div className="rounded-xl border border-white/[0.08] bg-[#090a0d]">
          ...
          {data.threads.slice(0, 5).map((thread) => (
            <button
              className="flex w-full items-center gap-3 border-b border-white/[0.05] px-4 py-3 text-left last:border-0 hover:bg-white/[0.025]"
              key={thread.id}
              onClick={() => onView("inbox")}
              type="button"
            >
              <span className="grid size-8 shrink-0 place-items-center rounded-full border border-white/10 bg-white/[0.04] text-[10px]">
        ...
        <div className="rounded-xl border border-white/[0.08] bg-[#090a0d] p-5">
...
function EmptyMail() {
  return (
    <div className="grid min-h-48 place-items-center text-center">
      <div>
        <Mail className="mx-auto size-6 text-white/20" />
