...
export function MailMessageBody({ message }: { message: MailMessageDetail }) {
  if (message.html) {
    const document = `<!doctype html><html><head><meta name="color-scheme" content="light dark"><style>body{margin:0;padding:16px;background:#f8f9fa;color:#030303;font:14px/1.65 system-ui,sans-serif}a{color:#A855F7}img{max-width:100%;height:auto}@media (prefers-color-scheme: dark){body{background:#0b0c10;color:#d6d7db}a{color:#c084fc}}</style></head><body>${message.html}</body></html>`;
    return (
      <iframe
