...
export default function ParticleLogo({
  ...
  useEffect(() => {
    ...
    const sampleLogo = (): void => {
      ...
      // Draw the Multivrs "Peak M" brand mark scaled precisely
      const logoSize = Math.min(width, height) * scaleRatio;
      offscreen.width = Math.max(1, Math.floor(logoSize));
      offscreen.height = Math.max(1, Math.floor(logoSize));
