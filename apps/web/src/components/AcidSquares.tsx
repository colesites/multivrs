...
const AcidSquares: React.FC<AcidSquaresProps> = ({
  ...
  useEffect(() => {
    ...
    const renderFrame = () => {
      ...
      if (blurRef.current > 0) {
        ...
        if (mu.uGrain) mu.uGrain.value = grainOn;
  ...
  useEffect(() => {
    ...
    if (u.uSpread) u.uSpread.value = spread;
    if (u.uStepSize) u.uStepSize.value = stepSize;
    if (u.uGlow) u.uGlow.value = glow;
    if (u.uExposure) u.uExposure.value = exposure;
    if (u.uColorShift) u.uColorShift.value = colorShift;
    ...
    if (u.uSteps) u.uSteps.value = stepsFor(detail);
    if (u.uMouseRadius) u.uMouseRadius.value = mouseRadius;

    const c1 = hexToRgb(color1);
    const a1 = u.uColor1?.value as Float32Array | undefined;
    if (a1) {
      a1[0] = c1[0];
      a1[1] = c1[1];
      a1[2] = c1[2];
    ...
    const a2 = u.uColor2?.value as Float32Array | undefined;
    if (a2) {
      a2[0] = c2[0];
      a2[1] = c2[1];
      a2[2] = c2[2];
    ...
    const a3 = u.uColor3?.value as Float32Array | undefined;
    if (a3) {
      a3[0] = c3[0];
      a3[1] = c3[1];
      a3[2] = c3[2];
